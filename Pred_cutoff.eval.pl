...
The eval.1 file contains all predictions with E value <= 1.0 and the eval.rest file contains rest of the 
predictions.
...
# Prep work
my ($rec,%counts,$ctr,$j,$k,@temp,$seq,$mlen,$highctr,$lowctr,%evals);
$i=localtime();
...
if($engine eq 'patser'){
	...
	#write out hitcounts.txt and allhits.txt with pseudocount 1
	unless(open(HC,">hitcounts.txt")){print "not able to open hitcounts.txt\n\n";exit 1;}
	unless(open(AH,">allhits.txt")){print "not able to open allhits.txt\n\n";exit 1;}
	my($key,$val);
	while (($key,$val) = each %counts){
		print HC "$key ",$val+1,"\n";
		for (0..$val){print AH "$key\n";}
	}
	close(AH); close(HC);
	
	#Run R script and read in score->eval mapping
	$k=system "R --slave --no-save --no-restore --no-environ --silent --args $copies < gpd.R";
	if($k!=0){ print STDERR "R script execution failed: $?\n"; exit 1;}
	unless(open(EV,"<Evals.out")){print "not able to open Evals.txt\n\n";exit 1;}
	while($rec=<EV>){
		chomp $rec; @temp=split(/\s+/,$rec);
		#if($temp[1] eq 'NaN'){$evals{sprintf("%.1f",$temp[0])}=0;}
		#else{$evals{sprintf("%.1f",$temp[0])}=$temp[1];}
		$evals{sprintf("%.1f",$temp[0])}=$temp[1];
	}
	close(EV); unlink 'Evals.out'; unlink 'hitcounts.txt'; unlink 'allhits.txt';
	 
	#make new gff file with predictions and E values
	if($verbose){print STDERR "\nCreating GFF file with predictions..\n\n";}
	unless(open(OUT1,">eval.1.$report")){print "not able to open eval.1.$report\n";exit 1;}
	unless(open(OUT2,">eval.rest.$report")){print "not able to open eval.rest.$report\n";exit 1;}
	...
	while($rec=<PSGFF>){
		if($rec =~ /^#/){ print OUT1 $rec; print OUT2 $rec; next;}
			...
			chomp $rec;
			@temp = split("\t",$rec);
			#if(($counts{sprintf("%.2f",$temp[5])}/$copies) > 1){
			if($evals{sprintf("%.1f",$temp[5])} > 1){
				$highctr++;
				print OUT2 $rec," Eval ",$evals{sprintf("%.1f",$temp[5])},";\n";
			}			
			else{
				$lowctr++;
				print OUT1 $rec," Eval ",$evals{sprintf("%.1f",$temp[5])},";\n";
			}
			@temp=();
		}
	}	
	...
	#read in hmmsearch report GFF for actual genome, make new gff file with E values
	if($verbose){print STDERR "\nCreating GFF file with predictions..\n\n";}
	unless(open(OUT1,">eval.1.$report")){print "not able to open eval.1.$report\n";exit 1;}
	unless(open(OUT2,">eval.rest.$report")){print "not able to open eval.rest.$report\n";exit 1;}
